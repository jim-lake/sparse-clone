...
const block_size = argv.block_size ? parseInt(argv.block_size) : 10 * 16 * 1024;
...
async function _run() {
  ...
  try {
    ...
    const buffer0 = Buffer.alloc(1, 0);
    const buffer42 = Buffer.alloc(1, 0x42);
    for (let i = 0; i < size; i += block_size) {
      const buf = Math.random() > 0.5 ? buffer0 : buffer42;
      await handle.write(buf, 0, 1, i);
    }
    await handle.close();
